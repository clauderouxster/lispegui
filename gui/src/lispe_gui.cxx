...
Element* Fltk_widget::labeltype(LispE* lisp) {
    ...
    Element* labeltype = lisp->get(U"thetype");
    if (labeltype == null_)
...
Element* Fltk_widget::labelfont(LispE* lisp) {
    ...
    Element* labelfont = lisp->get(U"font");
    if (labelfont == null_)
...
#endif
    Element* t = lisp->get(U"text");
    Element* x = lisp->get(U"x");
    Element* y = lisp->get(U"y");
...
void Fltk_widget::rectangle(LispE* lisp) {
    ...
    Element* x = lisp->get(U"x");
    Element* y = lisp->get(U"y");
    Element* wx = lisp->get(U"wx");
    Element* hy = lisp->get(U"hy");
    Element* ke = lisp->get(U"color");
...
void Fltk_widget::pie(LispE* lisp) {
    ...
    Element* x = lisp->get(U"x");
    Element* y = lisp->get(U"y");
    Element* w = lisp->get(U"w");
    Element* h = lisp->get(U"h");
    Element* a1 = lisp->get(U"a1");
    Element* a2 = lisp->get(U"a2");
    locking(lisp);
...
void Fltk_widget::circle(LispE* lisp) {
    if (widget == NULL)
        throw new Error("Error: Window does not exist");
    int x = lisp->get(U"x")->asInt();
    int y = lisp->get(U"y")->asInt();
    int r = lisp->get(U"r")->asInt();
    Element* kcolor = lisp->get(U"color");
...
void Fltk_widget::lineShape(LispE* lisp) {
    if (widget == NULL)
        throw new Error("Error: Window does not exist");
    int ke = lisp->get(U"type_shape")->asInt();
    int w = lisp->get(U"w")->asInt();
...
void Fltk_widget::line(LispE* lisp) {
    ...
    Element* x = lisp->get(U"x");
    Element* y = lisp->get(U"y");
    Element* x1 = lisp->get(U"x1");
    Element* y1 = lisp->get(U"y1");
    Element* x2 = lisp->get(U"x2");
    Element* y2 = lisp->get(U"y2");
    locking(lisp);
...
void Fltk_widget::textfont(LispE* lisp) {
    if (widget == NULL)
        throw new Error(L"Error: Widget not initialized");
    Element* f = lisp->get(U"f");
    Element* sz = lisp->get(U"sz");
    int i = f->asInt();
...
Element* Fltk_widget::rgbcolor(LispE* lisp) {
    if (widget == NULL)
        throw new Error(L"Error: Widget not initialized");
    Element* r = lisp->get(U"r");
    Element* g = lisp->get(U"g");
    Element* b = lisp->get(U"b");
...
Element* Fltk_widget::coordinates(LispE* lisp) {
    ...
    Element* xx = lisp->get(U"x");
    Element* yy = lisp->get(U"y");
    Element* ww = lisp->get(U"w");
    Element* hh = lisp->get(U"h");
...
void Fltk_widget::polygon(LispE* lisp) {
    ...
    Element* x2 = lisp->get(U"x2");
    Element* y2 = lisp->get(U"y2");
    Element* x3 = lisp->get(U"x3");
    Element* y3 = lisp->get(U"y3");
...
Element* Fltk_widget::linerotation(LispE* lisp) {
    if (widget == NULL)
        throw new Error(L"Error: Widget not initialized");
    double x = lisp->get(U"x")->asNumber();
    double y = lisp->get(U"y")->asNumber();
    double distance = lisp->get(U"distance")->asNumber();
    double angle = lisp->get(U"angle")->asNumber();
    bool draw = lisp->get(U"draw")->Boolean();
...
Element* Fltk_widget::plot(LispE* lisp) {
    ...
    Element* points = lisp->get(U"points");
    if (!points->isList())
        throw new Error(L"WND(872): We expect a list as first parameter.");
    long thickness = lisp->get(U"thickness")->asInt();
    ...
    double incy = 0.0;
    char action = 0;
    Element* klandmark = lisp->get(U"landmark");
...
Element* Fltk_widget::plotcoords(LispE* lisp) {
    Element* kvect = lisp->get(U"landmark");
    if (kvect->size() != 11)
...
void Fltk_input::insert(LispE* lisp) {
    ...
    buf = lisp->get(U"val")->toString(lisp);
    int pos = lisp->get(U"pos")->asInt();
    locking(lisp);
...
void Fltk_output::wrap() {
    ...
    bool mode = lisp->get(U"mode")->Boolean();
    locking(lisp);
    if (mode)
...
void Fltk_slider::boundaries() {
    ...
    double x = lisp->get(U"low")->asNumber();
    double y = lisp->get(U"high")->asNumber();
    locking(lisp);
...
void Fltk_slider::step() {
    if (widget == NULL)
        throw new Error("WND(101): No slider available");
    double stp = lisp->get(U"stp")->asNumber();
    locking(lisp);
...
void Fltk_window::finalize(LispE* lisp) {
    ...
    Element* timer = lisp->get(U"timer");
    if (timer != null_) {
...
void Fltk_window::gif_image(LispE* lisp) {
    ...
    int x = lisp->get(U"x")->asInt();
    int y = lisp->get(U"y")->asInt();
    int wx = lisp->get(U"wx")->asInt();
    int wy = lisp->get(U"wy")->asInt();
...
void Fltk_window::resize() {
    if (widget == NULL)
        throw new Error(L"Error: Widget not initialized");
    int minw = lisp->get(U"minw")->asInt();
    int minh = lisp->get(U"minh")->asInt();
    int maxw = lisp->get(U"maxw")->asInt();
    int maxh = lisp->get(U"maxh")->asInt();
    window()->size_range(minw, minh, maxw, maxh);
}

Element* Fltk_window::ask(LispE* lisp) {
    int a;
    string msg = lisp->get(U"msg")->toString(lisp);
    string msg1 = lisp->get(U"msg1")->toString(lisp);
    string msg2 = lisp->get(U"msg2")->toString(lisp);
    Element* emsg3 = lisp->get(U"msg3");
    Element* emsg4 = lisp->get(U"msg4");
    locking(lisp);
    if (emsg3 == null_)
    ...
    try {
        switch (action) {
            case fltk_create: {
                ...
                int y = lisp->get(U"y")->asInt();
                int w = lisp->get(U"w")->asInt();
                int h = lisp->get(U"h")->asInt();
                string label = lisp->get(U"label")->toString(lisp);
                Element* function  = lisp->get(U"function");
                Element* object  = lisp->get(U"object");
            ...
            case fltk_input: {
                ...
                int w = lisp->get(U"w")->asInt();
                int h = lisp->get(U"h")->asInt();
                bool multiline = lisp->get(U"multiline")->Boolean();
                string label = lisp->get(U"label")->toString(lisp);
                Element* function  = lisp->get(U"function");
                Element* object  = lisp->get(U"object");
            ...
            case fltk_button: {
                ...
                int w = lisp->get(U"w")->asInt();
                int h = lisp->get(U"h")->asInt();
                int thetype = lisp->get(U"button_type")->asInt();
                int shape = lisp->get(U"button_shape")->asInt();
                string label = lisp->get(U"label")->toString(lisp);
                Element* function  = lisp->get(U"function");
                Element* object  = lisp->get(U"object");
            ...
            case fltk_slider: {
                ...
                int w = lisp->get(U"w")->asInt();
                int h = lisp->get(U"h")->asInt();
                int align = lisp->get(U"slider_orientation")->asInt();
                bool value_slider = lisp->get(U"slider_value_type")->Boolean();
                string label = lisp->get(U"label")->toString(lisp);
                Element* function  = lisp->get(U"function");
                Element* object  = lisp->get(U"object");
            ...
            case fltk_create_bitmap: {
                Element* kbitmaps = lisp->get(U"bitmap");
                if (!kbitmaps->isList())
                    throw new Error("Error: The argument should be a list of integers in 'fltk_create_bitmap'");

                int w = lisp->get(U"length")->asInt();
                int h = lisp->get(U"height")->asInt();
...
Exporting bool InitialisationModule(LispE* lisp) {
    ...
    lisp->recordingglobal(U"FL_FOREGROUND_COLOR", lisp->provideInteger(FL_FOREGROUND_COLOR));
    lisp->recordingglobal(U"FL_BACKGROUND2_COLOR", lisp->provideInteger(FL_BACKGROUND2_COLOR));
    lisp->recordingglobal(U"FL_BACKGROUND_COLOR", lisp->provideInteger(FL_BACKGROUND_COLOR));
    lisp->recordingglobal(U"FL_INACTIVE_COLOR", lisp->provideInteger(FL_INACTIVE_COLOR));
    lisp->recordingglobal(U"FL_SELECTION_COLOR", lisp->provideInteger(FL_SELECTION_COLOR));
    lisp->recordingglobal(U"FL_GRAY0", lisp->provideInteger(FL_GRAY0));
    lisp->recordingglobal(U"FL_DARK3", lisp->provideInteger(FL_DARK3));
    lisp->recordingglobal(U"FL_DARK2", lisp->provideInteger(FL_DARK2));
    lisp->recordingglobal(U"FL_DARK1", lisp->provideInteger(FL_DARK1));
    lisp->recordingglobal(U"FL_LIGHT1", lisp->provideInteger(FL_LIGHT1));
    lisp->recordingglobal(U"FL_LIGHT2", lisp->provideInteger(FL_LIGHT2));
    lisp->recordingglobal(U"FL_LIGHT3", lisp->provideInteger(FL_LIGHT3));
    lisp->recordingglobal(U"FL_BLACK", lisp->provideInteger(FL_BLACK));
    lisp->recordingglobal(U"FL_RED", lisp->provideInteger(FL_RED));
    lisp->recordingglobal(U"FL_GREEN", lisp->provideInteger(FL_GREEN));
    lisp->recordingglobal(U"FL_YELLOW", lisp->provideInteger(FL_YELLOW));
    lisp->recordingglobal(U"FL_BLUE", lisp->provideInteger(FL_BLUE));
    lisp->recordingglobal(U"FL_MAGENTA", lisp->provideInteger(FL_MAGENTA));
    lisp->recordingglobal(U"FL_CYAN", lisp->provideInteger(FL_CYAN));
    lisp->recordingglobal(U"FL_DARK_RED", lisp->provideInteger(FL_DARK_RED));
    lisp->recordingglobal(U"FL_DARK_GREEN", lisp->provideInteger(FL_DARK_GREEN));
    lisp->recordingglobal(U"FL_DARK_YELLOW", lisp->provideInteger(FL_DARK_YELLOW));
    lisp->recordingglobal(U"FL_DARK_BLUE", lisp->provideInteger(FL_DARK_BLUE));
    lisp->recordingglobal(U"FL_DARK_MAGENTA", lisp->provideInteger(FL_DARK_MAGENTA));
    lisp->recordingglobal(U"FL_DARK_CYAN", lisp->provideInteger(FL_DARK_CYAN));
    lisp->recordingglobal(U"FL_WHITE", lisp->provideInteger(FL_WHITE));

    lisp->recordingglobal(U"FL_SOLID",lisp->provideInteger(FL_SOLID));
    lisp->recordingglobal(U"FL_DASH",lisp->provideInteger(FL_DASH));
    lisp->recordingglobal(U"FL_DOT",lisp->provideInteger(FL_DOT));
    lisp->recordingglobal(U"FL_DASHDOT",lisp->provideInteger(FL_DASHDOT));
    lisp->recordingglobal(U"FL_DASHDOTDOT",lisp->provideInteger(FL_DASHDOTDOT));
    lisp->recordingglobal(U"FL_CAP_FLAT",lisp->provideInteger(FL_CAP_FLAT));
    lisp->recordingglobal(U"FL_CAP_ROUND",lisp->provideInteger(FL_CAP_ROUND));
    lisp->recordingglobal(U"FL_CAP_SQUARE",lisp->provideInteger(FL_CAP_SQUARE));
    lisp->recordingglobal(U"FL_JOIN_MITER",lisp->provideInteger(FL_JOIN_MITER));
    lisp->recordingglobal(U"FL_JOIN_ROUND",lisp->provideInteger(FL_JOIN_ROUND));
    lisp->recordingglobal(U"FL_JOIN_BEVEL",lisp->provideInteger(FL_JOIN_BEVEL));

    lisp->recordingglobal(U"FL_VERT_SLIDER", lisp->provideInteger(FL_VERT_SLIDER));
    lisp->recordingglobal(U"FL_HOR_SLIDER", lisp->provideInteger(FL_HOR_SLIDER));
    lisp->recordingglobal(U"FL_VERT_FILL_SLIDER", lisp->provideInteger(FL_VERT_FILL_SLIDER));
    lisp->recordingglobal(U"FL_HOR_FILL_SLIDER", lisp->provideInteger(FL_HOR_FILL_SLIDER));
    lisp->recordingglobal(U"FL_VERT_NICE_SLIDER", lisp->provideInteger(FL_VERT_NICE_SLIDER));
    lisp->recordingglobal(U"FL_HOR_NICE_SLIDER", lisp->provideInteger(FL_HOR_NICE_SLIDER));

    lisp->recordingglobal(U"FL_HELVETICA", lisp->provideInteger(0));
    lisp->recordingglobal(U"FL_HELVETICA_BOLD", lisp->provideInteger(1));
    lisp->recordingglobal(U"FL_HELVETICA_ITALIC", lisp->provideInteger(2));
    lisp->recordingglobal(U"FL_HELVETICA_BOLD_ITALIC", lisp->provideInteger(3));
    lisp->recordingglobal(U"FL_COURIER", lisp->provideInteger(4));
    lisp->recordingglobal(U"FL_COURIER_BOLD", lisp->provideInteger(5));
    lisp->recordingglobal(U"FL_COURIER_ITALIC", lisp->provideInteger(6));
    lisp->recordingglobal(U"FL_COURIER_BOLD_ITALIC", lisp->provideInteger(7));
    lisp->recordingglobal(U"FL_TIMES", lisp->provideInteger(8));
    lisp->recordingglobal(U"FL_TIMES_BOLD", lisp->provideInteger(9));
    lisp->recordingglobal(U"FL_TIMES_ITALIC", lisp->provideInteger(10));
    lisp->recordingglobal(U"FL_TIMES_BOLD_ITALIC", lisp->provideInteger(11));
    lisp->recordingglobal(U"FL_SYMBOL", lisp->provideInteger(12));
    lisp->recordingglobal(U"FL_SCREEN", lisp->provideInteger(13));
    lisp->recordingglobal(U"FL_SCREEN_BOLD", lisp->provideInteger(14));
    lisp->recordingglobal(U"FL_ZAPF_DINGBATS", lisp->provideInteger(15));
    lisp->recordingglobal(U"FL_FREE_FONT", lisp->provideInteger(16));
    lisp->recordingglobal(U"FL_BOLD", lisp->provideInteger(1));
    lisp->recordingglobal(U"FL_ITALIC", lisp->provideInteger(2));
    lisp->recordingglobal(U"FL_BOLD_ITALIC", lisp->provideInteger(3));

    lisp->recordingglobal(U"FL_REGULAR_BUTTON_TYPE", lisp->provideInteger(0));
    lisp->recordingglobal(U"FL_CHECK_BUTTON_TYPE", lisp->provideInteger(1));
    lisp->recordingglobal(U"FL_LIGHT_BUTTON_TYPE", lisp->provideInteger(2));
    lisp->recordingglobal(U"FL_REPEAT_BUTTON_TYPE", lisp->provideInteger(3));
    lisp->recordingglobal(U"FL_RETURN_BUTTON_TYPE", lisp->provideInteger(4));
    lisp->recordingglobal(U"FL_ROUND_BUTTON_TYPE", lisp->provideInteger(5));
    lisp->recordingglobal(U"FL_IMAGE_BUTTON_TYPE", lisp->provideInteger(6));

    lisp->recordingglobal(U"FL_NORMAL_BUTTON_SHAPE", lisp->provideInteger(FL_NORMAL_BUTTON));
    lisp->recordingglobal(U"FL_TOGGLE_BUTTON_SHAPE", lisp->provideInteger(FL_TOGGLE_BUTTON));
    lisp->recordingglobal(U"FL_RADIO_BUTTON_SHAPE", lisp->provideInteger(FL_RADIO_BUTTON));
    lisp->recordingglobal(U"FL_HIDDEN_BUTTON_SHAPE", lisp->provideInteger(FL_HIDDEN_BUTTON));

    lisp->recordingglobal(U"FL_WHEN_NEVER", lisp->provideInteger(FL_WHEN_NEVER));
    lisp->recordingglobal(U"FL_WHEN_CHANGED", lisp->provideInteger(FL_WHEN_CHANGED));
    lisp->recordingglobal(U"FL_WHEN_RELEASE", lisp->provideInteger(FL_WHEN_RELEASE));
    lisp->recordingglobal(U"FL_WHEN_RELEASE_ALWAYS", lisp->provideInteger(FL_WHEN_RELEASE_ALWAYS));
    lisp->recordingglobal(U"FL_WHEN_ENTER_KEY", lisp->provideInteger(FL_WHEN_ENTER_KEY));
    lisp->recordingglobal(U"FL_WHEN_ENTER_KEY_ALWAYS", lisp->provideInteger(FL_WHEN_ENTER_KEY_ALWAYS));

    lisp->recordingglobal(U"FL_ALIGN_CENTER", lisp->provideInteger(FL_ALIGN_CENTER));
    lisp->recordingglobal(U"FL_ALIGN_TOP", lisp->provideInteger(FL_ALIGN_TOP));
    lisp->recordingglobal(U"FL_ALIGN_BOTTOM", lisp->provideInteger(FL_ALIGN_BOTTOM));
    lisp->recordingglobal(U"FL_ALIGN_LEFT", lisp->provideInteger(FL_ALIGN_LEFT));
    lisp->recordingglobal(U"FL_ALIGN_RIGHT", lisp->provideInteger(FL_ALIGN_RIGHT));
    lisp->recordingglobal(U"FL_ALIGN_INSIDE", lisp->provideInteger(FL_ALIGN_INSIDE));
    lisp->recordingglobal(U"FL_ALIGN_TEXT_OVER_IMAGE", lisp->provideInteger(FL_ALIGN_TEXT_OVER_IMAGE));
    lisp->recordingglobal(U"FL_ALIGN_IMAGE_OVER_TEXT", lisp->provideInteger(FL_ALIGN_IMAGE_OVER_TEXT));
    lisp->recordingglobal(U"FL_ALIGN_CLIP", lisp->provideInteger(FL_ALIGN_CLIP));
    lisp->recordingglobal(U"FL_ALIGN_WRAP", lisp->provideInteger(FL_ALIGN_WRAP));
    lisp->recordingglobal(U"FL_ALIGN_IMAGE_NEXT_TO_TEXT", lisp->provideInteger(FL_ALIGN_IMAGE_NEXT_TO_TEXT));
    lisp->recordingglobal(U"FL_ALIGN_TEXT_NEXT_TO_IMAGE", lisp->provideInteger(FL_ALIGN_TEXT_NEXT_TO_IMAGE));
    lisp->recordingglobal(U"FL_ALIGN_IMAGE_BACKDROP", lisp->provideInteger(FL_ALIGN_IMAGE_BACKDROP));
    lisp->recordingglobal(U"FL_ALIGN_TOP_LEFT", lisp->provideInteger(FL_ALIGN_TOP_LEFT));
    lisp->recordingglobal(U"FL_ALIGN_TOP_RIGHT", lisp->provideInteger(FL_ALIGN_TOP_RIGHT));
    lisp->recordingglobal(U"FL_ALIGN_BOTTOM_LEFT", lisp->provideInteger(FL_ALIGN_BOTTOM_LEFT));
    lisp->recordingglobal(U"FL_ALIGN_BOTTOM_RIGHT", lisp->provideInteger(FL_ALIGN_BOTTOM_RIGHT));
    lisp->recordingglobal(U"FL_ALIGN_LEFT_TOP", lisp->provideInteger(FL_ALIGN_LEFT_TOP));
    lisp->recordingglobal(U"FL_ALIGN_RIGHT_TOP", lisp->provideInteger(FL_ALIGN_RIGHT_TOP));
    lisp->recordingglobal(U"FL_ALIGN_LEFT_BOTTOM", lisp->provideInteger(FL_ALIGN_LEFT_BOTTOM));
    lisp->recordingglobal(U"FL_ALIGN_RIGHT_BOTTOM", lisp->provideInteger(FL_ALIGN_RIGHT_BOTTOM));
    lisp->recordingglobal(U"FL_ALIGN_NOWRAP", lisp->provideInteger(FL_ALIGN_NOWRAP));
    lisp->recordingglobal(U"FL_ALIGN_POSITION_MASK", lisp->provideInteger(FL_ALIGN_POSITION_MASK));
    lisp->recordingglobal(U"FL_ALIGN_IMAGE_MASK", lisp->provideInteger(FL_ALIGN_IMAGE_MASK));
